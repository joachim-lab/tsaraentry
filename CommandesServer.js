...
/* =============================================================
 * TEMPORARY - Item 3 timing. READ ONLY, writes nothing anywhere.
 * Remove once the slow stage is identified.
 * Run it from the editor with no arguments.
 * ============================================================= */
function timeItem3(orderKey) {
  const key = cmdCanonKey(orderKey || "14-C4");
  const marks = [];
  var last = Date.now();
  function mark(label) {
    const now = Date.now();
  ...
  const res = cmdGetReservation(key);
  mark("1 cmdGetReservation  (opens Commandes, reads R\u00e9servations) [" + res + "]");

  const list = getLotFileList();
  mark("2 getLotFileList  (Drive folder enumeration) [" + list.length + " files]");
  ...
  if (!fileId) { Logger.log("no lot file for lot number " + lotNum); return; }

  // getName() forces the fetch. openById on its own is lazy, so timing it
  // alone would report ~0 ms and push the real cost into the next stage.
  const lotSS = SpreadsheetApp.openById(fileId);
  const nm = lotSS.getName();
  mark("3 openById + getName  (forces the fetch) [" + nm + "]");

  const m = findSubLotColumnByOrderKey(lotSS, key);
  mark("4 findSubLotColumnByOrderKey  [source " + m.source + ", found " + m.found + "]");

  const sh = cmdSheet();
  const lastRow = sh.getLastRow();
  mark("5 cmdSheet + getLastRow  [lastRow " + lastRow + "]");

  const pending = cmdGetPendingQty(key);
  mark("6 cmdGetPendingQty  [" + (lastRow - CMD_CFG.START_ROW + 1) +
       " rows x 27 cols, pending " + pending + "]");

  Logger.log("=== timeItem3 : " + key + " ===");
  marks.forEach(function (s) { Logger.log("  " + s); });

  // Reference: the whole call as the UI actually makes it. Runs second, so
  // it benefits from any per-execution caching - expect it to be FASTER
  // than the sum above. That gap is itself information.
  const t0 = Date.now();
  cmdGetLotAvailability(key);
  Logger.log("  TOTAL cmdGetLotAvailability (warm) \u2014 " + (Date.now() - t0) + " ms");
}
